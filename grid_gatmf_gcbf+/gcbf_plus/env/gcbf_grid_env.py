...
class UnifiedGridEnv(MultiAgentEnv):
    ...
    def step(self, graph: GraphsTuple, action: Array) -> Tuple[GraphsTuple, Reward, Cost, Done, Info]:
        ...
        reward = float(-jnp.mean(jnp.sum((u_applied - uref) ** 2, axis=-1)))
        flags = _collision_flags(new_agent, obs)  # (n_agent,)
        cost = float(jnp.mean(flags.astype(jnp.float32)))

        done = (self._step_count >= self.max_steps) or bool(jnp.all(self.finish_mask(next_graph)))
        info = {
    ...
    def state_lim(self) -> Tuple[Array, Array]:
        ...
        ub = jnp.array([float(self.grid_size - 1), float(self.grid_size - 1)], dtype=jnp.float32)
        return lb, ub
    ...
    def _build_graph(self, env_state: EnvState) -> GraphsTuple:
        ...
        senders, receivers, e_feats = self._build_edges(env_state)
    ...
    def _build_edges(self, env_state: EnvState) -> Tuple[Array, Array, Array]:
        """
        ...
        s_AA, r_AA, f_AA = _mask_to_edges(ids_A, ids_A, diff_AA, mask_AA)

        # A->G (paired)
        diff_AG = A[:, None, :] - G[None, :, :]                   # (na, na, 2)
        mask_AG = jnp.eye(na, dtype=jnp.bool_)                    # only pair (i->i)
        s_AG, r_AG, f_AG = _mask_to_edges(ids_A, ids_G, diff_AG, mask_AG)
        ...
        if no > 0:
            ...
            mask_AO = (man_AO <= 2.0)
            s_AO, r_AO, f_AO = _mask_to_edges(ids_A, ids_O, diff_AO, mask_AO)
...
def _cont2disc(u: Array) -> Array:
    ...
    stay if ||u|| < 1e-6; else choose axis with larger |component| and sign.
    """
...
def _apply_discrete_moves(agent: Array, disc: Array, obs: Array, grid_size: int) -> Array:
    ...
    n = agent.shape[0]
    prop = jnp.clip(agent + dir_map[disc], 0.0, float(grid_size - 1))  # (n,2)
...
def _mask_to_edges(ids_recv: Array, ids_send: Array, diffs: Array, mask: Array) -> Tuple[Array, Array, Array]:
    ...
    """
    ii, jj = jnp.where(mask)                       # (E,), (E,)
    if ii.size == 0:
        return (jnp.zeros((0,), dtype=jnp.int32),
                jnp.zeros((0,), dtype=jnp.int32),
                jnp.zeros((0, diffs.shape[-1]), dtype=diffs.dtype))
    senders   = ids_send[jj]
    receivers = ids_recv[ii]
    feats     = diffs[ii, jj]                      # (E,2)
